...
Smart Customer Support Bot with Escalation  
A cloud-native chatbot built with **FastAPI**, **Dialogflow CX**, and **Salesforce API integration**.  
The bot handles basic customer queries using an FAQ knowledge base and escalates complex issues to human agents, automatically logging structured case details in Salesforce.
...
```mermaid
flowchart TD
    User --> ChatUI[Chatbot UI]
    ChatUI --> Backend[FastAPI Backend]
    Backend --> Intent["Intent Detection (Vertex AI or Dialogflow CX)"]
    Backend --> FAQ["FAQ Lookup (JSON/Firestore)"]
    Backend --> Escalation[Escalation Workflow]
    Escalation --> Salesforce["Salesforce API - Case Logging"]
    Backend --> CloudRun[Cloud Run Deployment]
    CloudRun --> CICD["CI/CD via GitHub Actions"]
```
---
